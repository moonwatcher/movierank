...
hadoop fs -rm -r fourth_bootstrap_* fourth_ranking_*> /dev/null 2>&1
...
#hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
#-input fourth_bootstrap_clean/part-00000 \
#-output fourth_bootstrap_zero_people \
...
#NUMBER_OF_PEOPLE=$(hadoop fs -cat fourth_bootstrap_zero_people/part-00000)
#echo "Found $NUMBER_OF_PEOPLE people nodes"

echo 'Counting content...'
hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
-input fourth_bootstrap_clean/part-00000 \
-output fourth_bootstrap_zero_content \
-mapper "bootstrap/zero/map.py content" \
-reducer bootstrap/zero/reduce.py \
-file bootstrap/zero/map.py \
-file bootstrap/zero/reduce.py

NUMBER_OF_CONTENT=$(hadoop fs -cat fourth_bootstrap_zero_content/part-00000)
echo "Found $NUMBER_OF_CONTENT content nodes"

echo 'Allocating uniform ranks to content'
hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
-input fourth_bootstrap_clean/part-00000 \
-output fourth_ranking_content \
...
hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
-input fourth_ranking_content/part-00000 \
-output fourth_ranking_people \
...
while [ $COUNTER -lt $ITERATIONS ]; do 
    ...
    hadoop fs -rm -r fourth_ranking_content > /dev/null 2>&1
    hadoop fs -rm -r fourth_ranking_sum > /dev/null 2>&1
    
    echo 'Calculating people ranking normalization factor'
    hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
    -input fourth_ranking_people/part-00000 \
    -output fourth_ranking_sum \
    -mapper ranking/sum/map.py \
    -reducer ranking/sum/reduce.py \
    -file ranking/sum/map.py \
    -file ranking/sum/reduce.py
    TOTAL_PEOPLE_RANK=$(hadoop fs -cat fourth_ranking_sum/part-00000|cut -f 2)
    ...
    hadoop fs -rm -r fourth_ranking_people > /dev/null 2>&1
    hadoop fs -rm -r fourth_ranking_sum > /dev/null 2>&1
    ...
    -file ranking/sum/map.py \
    -file ranking/sum/reduce.py
    TOTAL_CONTENT_RANK=$(hadoop fs -cat fourth_ranking_sum/part-00000|cut -f 1)
...
TOTALS="$(hadoop fs -cat fourth_ranking_sum/part-00000)"
...
hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
-input fourth_ranking_people/part-00000 \
-output fourth_ranking_normalized \
...
hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
-input fourth_ranking_normalized/part-00000 \
-output fourth_ranking_content_result \
...
hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar \
-input fourth_ranking_normalized/part-00000 \
-output fourth_ranking_people_result \
...
hadoop fs -copyToLocal fourth_ranking_content_result/part-00000 movie.rank
hadoop fs -copyToLocal fourth_ranking_people_result/part-00000 people.rank
cat movie.rank|sort -rn > movie.rank.sorted
cat people.rank|sort -rn > people.rank.sorted

# hadoop fs -copyToLocal fourth_ranking_people/part-00000 movie.raw
